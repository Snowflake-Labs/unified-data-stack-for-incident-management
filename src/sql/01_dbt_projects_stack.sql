...
create or replace database role <% ctx.env.dbt_project_database %>.manage_landing_zone;
create or replace database role <% ctx.env.dbt_project_database %>.manage_curated_zone;
create or replace database role <% ctx.env.dbt_project_database %>.manage_dbt_deployments;

grant usage on database <% ctx.env.dbt_project_database %> to database role <% ctx.env.dbt_project_database %>.manage_landing_zone;
grant create schema on database <% ctx.env.dbt_project_database %> to database role <% ctx.env.dbt_project_database %>.manage_landing_zone;

grant usage on database <% ctx.env.dbt_project_database %> to database role <% ctx.env.dbt_project_database %>.manage_curated_zone;
grant create schema on database <% ctx.env.dbt_project_database %> to database role <% ctx.env.dbt_project_database %>.manage_curated_zone;
...
grant database role manage_landing_zone to role <% ctx.env.dbt_project_admin_role %>;
grant database role manage_curated_zone to role <% ctx.env.dbt_project_admin_role %>;
...
grant all privileges on schema <% ctx.env.dbt_project_database %>.landing_zone to database role <% ctx.env.dbt_project_database %>.manage_landing_zone;
...
CREATE DBT PROJECT <% ctx.env.dbt_project_database %>.dbt_project_deployments.<% ctx.env.dbt_project_name %>
  FROM '@<% ctx.env.dbt_project_database %>.dbt_project_deployments.incident_management_dbt_project_repo/tags/v1.0.0/src/incident_management'
...
alter task if exists dbt_project_deployments.im_project_run_select_landing_zone suspend;
alter task if exists dbt_project_deployments.im_project_run_select_curated_zone suspend;
...
create or replace task dbt_project_deployments.im_project_run_select_landing_zone
	warehouse=<% ctx.env.dbt_snowflake_warehouse %>
...
create or replace task dbt_project_deployments.im_project_run_select_curated_zone
	warehouse=<% ctx.env.dbt_snowflake_warehouse %>
	after dbt_project_deployments.im_project_compile, dbt_project_deployments.im_project_run_select_landing_zone
	as 
  EXECUTE IMMEDIATE
  $$
    BEGIN
...
-- 	warehouse=<% ctx.env.dbt_snowflake_warehouse %>
-- 	after dbt_project_deployments.im_project_compile, dbt_project_deployments.im_project_run_select_landing_zone, dbt_project_deployments.im_project_run_select_curated_zone
-- 	as
--   EXECUTE IMMEDIATE 
--   $$
--     BEGIN
...
grant all privileges on schema <% ctx.env.dbt_project_database %>.curated_zone to database role <% ctx.env.dbt_project_database %>.manage_curated_zone;
