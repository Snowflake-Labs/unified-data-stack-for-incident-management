...
TABLES(
  doc_extracts as {{ ref('document_full_extracts') }}
    COMMENT = 'All policy documents'
  , users as {{ ref('users') }}
    COMMENT = 'Materialized users table with enriched data'
  , v_new_docs as {{ ref('v_qualify_new_documents') }}
  ...
  , v_slack_incident_msgs as {{ ref('v_slack_msg_incident_number') }}
    COMMENT = 'Slack messages with incident number, combining messages with and without explicit codes'
