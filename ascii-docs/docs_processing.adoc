...
The documents under _qa_ sub-directory are picked up by the dbt model link:../src/incident_management/models/silver_zone/document_question_extracts.sql[`document_question_extracts`] in Silver Zone. 
...
The `meta` configuration is defined in the dbt model link:../src/incident_management/models/silver_zone/document_question_extracts.yml[`document_question_extracts.yml`] in Silver Zone. It is a dictionary that contains the questions that will be used with the AI_EXTRACT function. The specific set of questions you will find is under the `quaterly_review_metrics` key, written for a quarterly review metrics reports (sample document: link:../data/documents/qa/incm_quarterly_review_metrics_2025Q3.pdf[incm_quarterly_review_metrics_2025Q3.pdf]).
