...
If you need to remove resources created by this project from your Snowflake account, a teardown script is provided.

[CAUTION]
====
*This operation is destructive and irreversible!*

The teardown script will permanently delete all objects *owned by `{default-role}`*:

* All schemas (bronze_zone, silver_zone, gold_zone, dbt_project_deployments)
* All tables and data within those schemas
* All Cortex services (Cortex Agent, Cortex Search Service, Semantic Views)
* All tasks, procedures, and functions
* All stages and their contents (including uploaded documents)
* Git repository and secrets

*Before proceeding:*

. Ensure you have backed up any data you need to retain
. Verify no other projects or users depend on these resources
====

=== Ownership Model

The teardown script respects the ownership model used during setup:
...
|`{default-role}`
|Schemas, tables, views, stages, streams, tasks, functions, DBT project, Git repository, secrets, Cortex services, Streamlit app

|`ACCOUNTADMIN`
|Database, warehouses, API integration, roles
|===

The main teardown script only drops objects owned by `{default-role}`. ACCOUNTADMIN-owned resources are listed in a separate commented section at the end of the script for optional manual cleanup.

=== Running the Teardown

The teardown script is located at `src/sql/06_teardown.sql`. It uses the same templating mechanism as the setup scripts. Use the Makefile target to run it with a confirmation prompt:

[source,bash,subs="attributes+"]
----
# Run the teardown (drops only dbt_project_admin_role-owned objects)
make teardown CONN=<{conn-placeholder}>
----

=== Teardown Phases

The script removes resources owned by `{default-role}` in the following order:
...
|1. Validation
|Pre-checks for active sessions and running queries

|2. Tasks
|All scheduled and triggered tasks (suspended then dropped)

|3. Cortex Services
|Cortex Agent, Cortex Search Service, Semantic Views

|4. Streamlit
|Dashboard application

|5. DBT Project
|DBT project, Git repository, and secrets

|6. Functions
|User-defined functions and procedures

|7. Tables
|All tables (respecting foreign key order)

|8. Streams
|Document streams

|9. Stages
|All stages and uploaded files

|10. Schemas
|All project schemas
|===

=== Complete Infrastructure Cleanup (Optional)

If you want to also remove ACCOUNTADMIN-owned resources (database, warehouses, integrations, roles), uncomment the relevant sections at the end of `src/sql/06_teardown.sql` and run the teardown again:

[source,bash,subs="attributes+"]
----
# After uncommenting ACCOUNTADMIN sections in 06_teardown.sql
make teardown CONN=<{conn-placeholder}>
----

[WARNING]
====
ACCOUNTADMIN-owned resources may be shared with other projects. Review carefully before dropping:

* *Warehouses* (`{default-warehouse}`, `document_search_service_wh`) - may be used by other applications
* *API Integration* - may be used for other Git repositories
* *Roles* - may have grants to other users or projects
====

[NOTE]
====
If you perform a partial teardown, be aware that some resources may have dependencies on others. Dropping schemas will automatically remove all objects within them (tables, views, stages, etc.).
====
