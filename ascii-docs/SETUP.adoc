...
----
+
Edit the `{env-file}` file and configure all variables except these two (generated later):
+
...
+
Once you have created the dbt Project, execute the Tasks available in the following order to build the baseline models and the Cortex AI services on top including the Cortex Agent:

* `incm_triggered_docs_processing`
